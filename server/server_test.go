...
// serve round-trip: POST /send delivers via the hook adapter to a captured callback with
// the reply threaded; GET /inbox?since=N returns appended messages; bearer auth is enforced.
func TestServe_SendThreadsAndInboxReadsSince(t *testing.T) {
	t.Setenv("HOOK_SECRET", "s")
	// A downstream that captures what messenger delivers on the "hook" channel.
	...
	cfg := &config.Config{Transports: map[string]config.Transport{
		"hook": {Kind: "hook", Enabled: true, TokenEnv: "HOOK_SECRET", Options: map[string]string{"callbackURL": cb.URL}},
	}}
	...
	s := New(cfg, box, transport.DefaultSenderRegistry(), transport.NewSecretResolver(nil), "tok", nil)
	srv := httptest.NewServer(s.Handler())
	defer srv.Close()

	// POST /send without the bearer → 401.
	noauth, _ := http.Post(srv.URL+"/send", "application/json", strings.NewReader(`{"channel":"hook","text":"x"}`))
	if noauth.StatusCode != http.StatusUnauthorized {
		t.Fatalf("want 401 without bearer, got %d", noauth.StatusCode)
	}

	// POST /send with bearer + reply_to → delivered, threaded.
	req, _ := http.NewRequest(http.MethodPost, srv.URL+"/send",
		strings.NewReader(`{"channel":"hook","text":"reply body","to":"chat-1","reply_to":"m-2"}`))
...
func mkEnv(id, text string) envelope.Envelope {
	return envelope.Normalize(envelope.Envelope{ID: id, Channel: "hook", Text: text})
}
